...
void LCD_PIN_Init(void){
	...
	// Enable the clock to GPIO Ports B
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN;
	// Enable the clock to GPIO Ports C
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN;
	// Enable the clock to GPIO Ports D
...
void LCD_Configure(void){
	...
	// Configure BIAS[1:0] bits of LCD_CR[6:5] and set the bias to 1/3 (10)
	LCD->CR &= ~(LCD_CR_BIAS);
	LCD->CR |= LCD_CR_BIAS_1;
	
	// Configure DUTY[2:0] bits of LCD_CR[4:2] and set the duty to 1/4 (011)
	LCD->CR &= ~(LCD_CR_DUTY);
	LCD->CR |= LCD_CR_DUTY_0 | LCD_CR_DUTY_1;
	
	// Set the LCD contrast to maximum (111) in LCD_FCR, which are bits LCD_FCR[12:10]
	LCD->FCR |= LCD_FCR_CC;
	
	// Set the LCD pulse on period to maximum (111) in LCD_FCR, which are bits LCD_FCR[6:4]
	LCD->FCR |= LCD_FCR_PON;
	
	// Disable the MUX Segment bit in LCD_CR, which is LCD_CR[7] bit
	LCD->CR &= ~(LCD_CR_MUX_SEG);
	
	// Set the LCD voltage source as the internal voltage source
	LCD->CR &= ~(LCD_CR_VSEL);
	
	// Wait until Register Synchronization Flag bit (FCRSF) is set, bit LCD_SR[5]
	while(!((LCD->SR)&(LCD_SR_FCRSF)));
	
	// Enable LCD, so set LCD_CR[0] = 1
	LCD->CR |= LCD_CR_LCDEN;
	
	// Wait until the LCD is enabled
	while(!((LCD->SR)&(LCD_SR_ENS)));
	
	// Wait until LCD booster (bit LCD_SR[4]) is ready
...
void LCD_Display_Time(uint32_t ms)
{
	...
	uint8_t disp_sec_1 = (seconds / 10) + 48;
	uint8_t disp_sec_0 = (seconds % 10) + 48;
	
	int minutes = ms / 60000;
	uint8_t disp_min_1 = (minutes / 10) + 48;
	uint8_t disp_min_0 = (minutes % 10) + 48;
	
	int disp_as_frac = (ms / 10) % 100;
	uint8_t disp_ms_1 = (disp_as_frac / 10) + 48;
	uint8_t disp_ms_0 = (disp_as_frac % 10) + 48;
	
	LCD_WriteChar(&disp_min_1, 0, 0, 0);
	LCD_WriteChar(&disp_min_0, 0, 1, 1);
	LCD_WriteChar(&disp_sec_1, 0, 0, 2);
	LCD_WriteChar(&disp_sec_0, 1, 0, 3);
	LCD_WriteChar(&disp_ms_1, 0, 0, 4);
	LCD_WriteChar(&disp_ms_0, 0, 0, 5);
}

void LCD_DispGame(uint32_t num, char* str){
	
	// Display letters
	LCD_WriteChar(&(str[0]), 0, 0, 0);
	LCD_WriteChar(&(str[1]), 0, 0, 1);
	LCD_WriteChar(&(str[2]), 0, 0, 2);
	LCD_WriteChar(&(str[3]), 0, 1, 3);
	
	// Convert num to characters
	char first = (num / 10) + 48;
	char second = (num % 10) + 48;
	
	// Display num
	LCD_WriteChar(&first, 0, 0, 4);
	LCD_WriteChar(&second, 0, 0, 5);
}
